...
class PartidaInline(admin.TabularInline):
    ...
    def ver_log_s3(self, obj):
        if obj.log_arquivo:
            # Aponta para a pasta media local em vez da AWS
            url = f"/{obj.log_arquivo}" 
            return format_html('<a href="{}" target="_blank" style="font-weight: bold; color: #007bff;">📄 Abrir Log</a>', url)
        return format_html('<span style="color: #999;">N/A</span>')
    ver_log_s3.short_description = "Link Arquivo" # Mudei o título para não ficar "Link AWS"
...
class ListandoHistoricoPartida(admin.ModelAdmin):
    ...
    def ver_log_clicavel(self, obj):
        if obj.log_arquivo:
            url = f"/{obj.log_arquivo}"
            return format_html('<a href="{}" target="_blank">🔗 Log Local</a>', url)
        return "Sem log"
